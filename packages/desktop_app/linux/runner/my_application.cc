...
struct _MyApplication {
  ...
  // Set when the login item launched us with --start-minimized, so the first
  // frame does not flash a window on its way to the tray.
...
static void my_application_activate(GApplication* application) {
  ...
  // A second launch must reach the daemon this process already owns rather
  // than start a competing one, so it only raises the existing window.
  ...
  // The brand tile color the boot splash also paints, so a cold launch never
  // shows a surface Flutter is about to replace with a different one.
...
MyApplication* my_application_new() {
  ...
  // machine, so any running Tinest — a developer's own app, or another
  ...
  if (g_getenv("TINYRACK_TINEST_ALLOW_MULTIPLE_INSTANCES") != nullptr) {

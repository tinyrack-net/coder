...
/// directory.
const String userMcpConfigFileName = 'mcp.json';

/// Filename holding project-scoped MCP configuration in a worktree root.
const String projectMcpConfigFileName = '.mcp.json';

/// The only document version this daemon reads.
const int mcpConfigVersion = 1;
...
/// Reads `<config>/mcp.json` and `<worktree root>/.mcp.json`.
///
  ...
  Future<void> save(McpConfigDocument document) async {
    if (document.scope == McpConfigScope.project) {
      throw const FormatException(
        'mcp_project_scope_readonly: a project .mcp.json belongs to the '
...
McpConfigDocument parseMcpConfig(
  ...
  if (decoded['version'] != mcpConfigVersion) {
    throw FormatException(
      'invalid_mcp_config: $sourcePath must declare "version": '
  ...
  for (final key in decoded.keys) {
    if (key != 'version' && key != 'servers') {
      throw FormatException(
        'invalid_mcp_config: $sourcePath has an unknown key "$key".',
      );
    }
  }
    ...
    for (final entry in entries.entries) {
      if (_namesASecret(entry.key) && !_carriesAReference(entry.value)) {
        reject(
          ...
          r'.mcp.json may only reference ${env:NAME} or ${secret:key}.',
        );
      }
    }
  }

...
/// One vendor compiled into the daemon.
///
...
abstract base class ProviderPlugin {
...
final class ProviderRegistry {
  ...
  factory ProviderRegistry({
    required List<ProviderPlugin> plugins,
    required List<ProviderWireProtocol> wireProtocols,
  }) {
    final pluginIds = <String>{};
    for (final plugin in plugins) {
      if (!pluginIds.add(plugin.id)) {
        throw StateError('Two provider plugins claim the id ${plugin.id}.');
      }
      if (plugin.definition.id != plugin.id) {
        throw StateError(
          'Plugin ${plugin.id} advertises the id ${plugin.definition.id}.',
        );
      }
    }
    ...
    return ProviderRegistry._(
      List<ProviderPlugin>.unmodifiable(plugins),
      List<ProviderWireProtocol>.unmodifiable(wireProtocols),
    );
  }

  const ProviderRegistry._(this.plugins, this.wireProtocols);
  ...
  ProviderPlugin? find(String definitionId) =>
      plugins.where((plugin) => plugin.id == definitionId).firstOrNull;

  /// Returns one vendor or fails for an unknown definition.
  ProviderPlugin require(String definitionId) {
    final plugin = find(definitionId);
    if (plugin == null) {

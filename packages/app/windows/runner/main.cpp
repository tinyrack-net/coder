...
int APIENTRY wWinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE prev,
  ...
  // Claimed before anything else so a duplicate launch costs nothing and never
  // reaches the embedded daemon, whose home lock a second process cannot take.
  SingleInstance single_instance;
  if (!single_instance.IsPrimary()) {
    SingleInstance::RevealRunningInstance();
    return EXIT_SUCCESS;
  }
  ...
  if (!window.Create(APP_DISPLAY_NAME, origin, size)) {
    return EXIT_FAILURE;
  }

...
`@tinyrack/ui/brand/apps/coder-app-icon.svg`, maintained in the
...
`coder.svg` in this directory is a verbatim copy of that master.
`coder-128.png`, `coder-256.png`, `coder-512.png`, and `coder-1024.png` are
rasterized from it and are what `shipworld.yaml` ships as the Linux desktop
icon.

## Native launcher icons

`coder-1024.png` is the source for every Android, iOS, macOS, Windows, and web
...
| File | Why it differs |
| --- | --- |
| `coder-adaptive-foreground.png` | Android composites it over the `#0a0a0a` adaptive background and applies its own 16% inset, so the tile must be absent. |

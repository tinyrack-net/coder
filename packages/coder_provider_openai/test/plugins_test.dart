...
void main() {
  ...
  test('the family registers each vendor and both wire protocols once', () {
    ...
    for (final plugin in registry.plugins) {
      ...
      expect(
        flows.contains(ProviderAuthFlow.none),
  ...
  test('only the subscription OAuth endpoint withholds model discovery', () {
    ...
    final platform = openai.endpoint(ProviderAuthKind.apiKey);
    ...
    final subscription = openai.endpoint(ProviderAuthKind.oauth);
    ...
    () async {
      ...
      final provider = openai.createProvider(
        ModelProviderRequest(
          connectionId: 'openai',
          endpoint: openai.endpoint(ProviderAuthKind.oauth),
